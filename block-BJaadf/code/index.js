...
let index1 = console.log(fruits[0]);
let index2 = console.log(fruits[1]);

// If the element at 0 index is undefined and index 1 is 'Banana' shift all the values to one lower index (use shift method)

if(index1 === undefined || index2 === "Banana"){
    fruits.shift();
}
...
fruits.map(fruits =>console.log(fruits.toLowerCase())); 
...
let doubleNumbers = numbersThree.forEach((num) => num *= 2)

// Create a new variable named tripleNumbers that store the numberThree array (each element should be multiplied by 3). Use map

let tripleNumbers = numbersThree.map(item => {
    return item * 3
})
...
let halfNumbers = numbersThree.map(item => {
    return item * 0.5
})
