...
function addTwoToArray(arrTwo){
  let newArr = [];
  for(let numbers of arrTwo){
     newArr.push(addTwo(numbers))
  }
}
/*
...
function addThreeToArray(arrThree){
  let newArr = [];
  for(let numbers of arrThree){
     newArr.push(addThree(numbers))
  }
}
...
/*
  Create a function named addFiveToArray which accepts:
    ...
    - while doing so use the funciton addThree
*/
function addFiveToArray(arrFive){
  let newArr = [];
  for(let numbers of arrFive){
     newArr.push(addFive(numbers))
  }
}
/*
...
*/
function changeArray(arrChange, fn){
  let arrNew = [];
  for(let numbers of arrChange){
    arrNew.push(fn(numbers));
  }
}
/*
...
*/
function isGradeB(gr){
  return gr.grade > 8 && gr.grade <= 12; 
}
/*
